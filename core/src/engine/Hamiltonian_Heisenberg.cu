...
namespace Engine
{
	...
	__inline__ __device__ int cu_get_pair_j(const int * boundary_conditions, const int * n_cells, int N, int ispin, const Pair * pair)
	{
		// Number of cells
		int Na = n_cells[0];
		int Nb = n_cells[1];
		...
		if ( ( boundary_conditions[0] || (0 <= nja && nja < n_cells[0]) ) &&
		     ( boundary_conditions[1] || (0 <= njb && njb < n_cells[1]) ) &&
		     ( boundary_conditions[2] || (0 <= njc && njc < n_cells[2]) ) )
		{
			// Boundary conditions fulfilled
			// Find the translations of spin j within the non-periodical domain
			if (pair->translations[0] < 0)
				nja += N*Na;
			if (pair->translations[1] < 0)
				njb += N*Na*Nb;
			if (pair->translations[2] < 0)
				njc += N*Na*Nb*Nc;
			// Calculate the correct index
			return (nja%Na)*N + (njb%Nb)*N*Na + (njc%Nc)*N*Na*Nb;
		}
		else
		{
	...
	__global__ void CU_E_Exchange(const Vector3 * spins, const int * boundary_conditions, const int * n_cells, int n_basis_spins, const scalar * Exchange_magnitude, const Pair * Exchange_pairs, int n_pairs, scalar * Energy, size_t size)
	{
			...
			ispin +=  blockDim.x * gridDim.x)
		{
			...
			{
				int jspin = cu_get_pair_j(boundary_conditions, n_cells, n_basis_spins, ispin, &Exchange_pairs[ipair]);
				if ( jspin >= 0 )
				{
					Energy[ispin] -= 0.5 * Exchange_magnitude[ipair] * spins[ispin].dot(spins[jspin]);
				}
			}
		}
	}
	...
	__global__ void CU_E_DMI(const Vector3 * spins, const int * boundary_conditions, const int * n_cells, int n_basis_spins, const scalar * DMI_magnitude, const Vector3 * DMI_normal, const Pair * DMI_pairs, int n_pairs, scalar * Energy, size_t size)
	{
			...
			ispin +=  blockDim.x * gridDim.x)
		{
			for (unsigned int ipair = 0; ipair < n_pairs; ++ipair)
			{
				int jspin = cu_get_pair_j(boundary_conditions, n_cells, n_basis_spins, ispin, &DMI_pairs[ipair]);
				if ( jspin >= 0 )
				{
					Energy[ispin] -= 0.5 *  DMI_magnitude[ipair] * DMI_normal[ipair].dot(spins[ispin].cross(spins[jspin]));
				}
			}
		}
	}
	...
	{
		int size = spins.size();
		CU_E_DMI<<<(size+1023)/1024, 1024>>>(spins.data(), boundary_conditions.data(), geometry->n_cells.data(), geometry->n_spins_basic_domain[0], DMI_magnitude.data(), DMI_normal.data(), DMI_pairs.data(), DMI_pairs.size(), Energy.data(), size);
	}
	...
	__global__ void CU_Gradient_Exchange(const Vector3 * spins, const int * boundary_conditions, const int * n_cells, int n_basis_spins, const scalar * Exchange_magnitude, const Pair * Exchange_pairs, int n_pairs, Vector3 * gradient, size_t size)
	{
			...
			ispin +=  blockDim.x * gridDim.x)
		{
			for (unsigned int ipair = 0; ipair < n_pairs; ++ipair)
			{
				...
				if ( jspin >= 0 )
				{
					gradient[ispin] -= Exchange_magnitude[ipair] * spins[jspin];
				}
			}
		}
	}
	...
	__global__ void CU_Gradient_DMI(const Vector3 * spins, const int * boundary_conditions, const int * n_cells, int n_basis_spins, const scalar * DMI_magnitude, const Vector3 * DMI_normal, const Pair * DMI_pairs, int n_pairs, Vector3 * gradient, size_t size)
	{
			...
			ispin +=  blockDim.x * gridDim.x)
		{
			for (unsigned int ipair = 0; ipair < n_pairs; ++ipair)
			{
				...
				if ( jspin >= 0 )
				{
					gradient[ispin] -= DMI_magnitude[ipair]*spins[jspin].cross(DMI_normal[ipair]);
				}
			}
		}
	}
