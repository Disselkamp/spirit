...
with state.State("input/"+cfgfile+".cfg") as p_state:
    ...
    for stt_magnitude in [ float(i) for i in sys.argv[5:] ]:  # read stt magnitude from terminal input
        ...
        file_positions.write('Step    Position(3)    weight(3)')
        ...
        for i in range(0, steps):
            ...
            m = sorted([spin[2] for spin in spins[-N*N:]])[:3]  # find spin with smallest z-component (of spins lying in the +z-surface)

            leng = len(spins[-N*N:])
            p0, p1, p2 = leng, leng, leng
            mges = sum(m)
            for j in range(0, leng):  # search and save position of the skyrmiontube/bobber
                if spins[j-N*N][2] == m[0]: p0 = j
                if spins[j-N*N][2] == m[1]: p1 = j
                if spins[j-N*N][2] == m[2]: p2 = j

                if p0 != leng and p1 != leng and p2 != leng:
                    file_positions.write('\n'+str(i)+'    '+str(p0)+' '+str(p1)+' '+str(p2)+'    '+str(m[0])+' '+str(m[1])+' '+str(m[2]))
                    print(str(i)+'    '+str(p0)+' '+str(p1)+' '+str(p2)+'    '+str(m[0])+' '+str(m[1])+' '+str(m[2]))
                    break
